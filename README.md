...
- The drawings use the formulas Hamid Naderi Yeganeh published in [“Making Mathematical Art”](https://www.scientificamerican.com/article/making-mathematical-art/) (*Scientific American*): 14,000, 12,000, 10,000 and 9,000 circles, 8,000 and 4,000 line segments, and 8,000 and 7,000 arcs. The page draws each element from the published formula for its centre, radius or end points (`src/figures.js`). A slow clock turns the inner terms, so the figures keep flowing; they lean toward the cursor and change to the next formula when clicked. The opening figure follows Naderi Yeganeh's animation *15,000 Moving Circles*: a four-fold chain of circles curling into loops, always in motion, turning slightly with the pointer, and passing slowly through a series of colour sets. The other figures each use a two- or three-colour set drawn from the owner's palette (#0FA077, #3ED900, #E131BD, #EB807D, #500140, #14353F, #2F42E8, #BA9F0D, #956BCF, #F748DB); on the dark page the deepest tones give way to white. The opening figure is large on every screen size. The artwork is credited to Hamid Naderi Yeganeh.
...
- Every post ends with a like, a share button and the comments:
  ...
  - **Comments:** listed from the post's Blogger comment feed. *Write a comment* opens Blogger's own comment form in a small window, because Google sign-in cannot run inside an embedded frame; the list refreshes when that window closes. When Blogger renders its native form, it stays, with a fallback link beneath it.
  - **Read more:** up to four other pieces, preferring the same chapter, beside a small string drawing of 72 chords (each point on a circle joined to the point at three times its angle).
- Every post ends with a small rights line (credit the author, link the original, reuse governed by the Copyright and Content Use policy, absolutely no AI training), and the footer on every page repeats *Absolutely no AI training*. The page source also carries a copyright and AI-use notice for crawlers and language models: an HTML comment, `noai` robots meta tags and schema.org `usageInfo`; none of it is visible to readers.
- The menu links to the page *About the line art used in this website*.
- Posts may carry their own styles and scripts (for example an animated poem). They run whether Blogger or the feed fallback renders the post, their CSS never leaks into excerpts, and dark inline text colours are lifted on the dark theme so they stay readable.
