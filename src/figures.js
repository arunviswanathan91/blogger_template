...
(() => {
  ...
  const FORMS = {
    ...
    moving15: { n: 15000, keep: .75, fit: .98, lw: .5, cycle: true,
      ...
      hue: (s, t) => { const phi = 21 * TAU * s; return Math.floor((((1.5 * (.5 - .5 * cos(2 * TAU * s)) + .18 * cos(4 * phi) ) % 1) + 1) % 1 * P); } },
  ...
  // Every element keeps its colour group; the groups' colours travel smoothly through the palette instead.
  // (Moving elements between groups each frame made large, zoomed-in circles blink from one colour to the next.)
  const bucketColor = (pal, g, shift) => {
    const n = pal.length, u = ((g / P * n + shift) % n + n) % n, i = Math.floor(u), k = Math.min(1, Math.max(0, (u - i - .55) / .45));
    return mixed(rgb(pal[i]), rgb(pal[(i + 1) % n]), k * k * (3 - 2 * k));
  };
  ...
  const paint = (f, formName, alpha, zoom = 1) => {
    ...
    if (form.custom) form.custom(f, groups);
      ...
      for (let k = 0; k < count; k++) {
        const s = k / n, e = form.el(s, f.t, f.m), path = groups[form.hue(s, 0)];
        if (e[0] === 'c') {
    ...
    groups.forEach((path, g) => {
      ...
      ctx.strokeStyle = form.cycle ? cycled[g] : form.custom ? (g < P ? pal[Math.floor(g * pal.length / P)] : colors.ink) : bucketColor(pal, g, f.t * .12 * pal.length);
  ...
  const tick = (time) => {
    ...
    frames.forEach((f) => {
      ...
      if (motionOK) {
        ...
        const calm = 1 / (f.zoom || 1) ** 2;
        f.m = { x: f.m.x + (goal.x * calm - f.m.x) * .05, y: f.m.y + (goal.y * calm - f.m.y) * .05 };
        if (f.zooms) {
        ...
        // A zooming figure repaints every frame: switching to a slower pace mid-zoom is what made it stutter when magnified.
        if (f.zooms || time - (f.painted || 0) > ((f.cost || 0) < 9 ? 0 : 31)) changed = true;
      }
